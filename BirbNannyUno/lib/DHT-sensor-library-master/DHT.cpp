...
Adafruit_MCP23XXX mcp = Adafruit_MCP23XXX();
...
bool usingMcp = false; // flag to indicate if we are using an MCP23017
 ...
 *          number of sensors
 */
DHT::DHT(uint8_t pin, uint8_t type, bool useMcp) {
  _pin = pin;
  _type = type;
#ifdef __AVR
  ...
  usingMcp = useMcp; // set the flag to indicate if we are using an MCP23017
